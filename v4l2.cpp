...
BUF *v4l2(char *FILE_VIDEO, BUF *buffers)
{
    ...
    int ret = 0;
    //open dev
    if ((buffers[0].fd = open(FILE_VIDEO, O_RDWR)) == -1)
    {
    ...
    //query cap
    if (ioctl(buffers[0].fd, VIDIOC_QUERYCAP, &cap) == -1)
    {
    ...
    while(ioctl(buffers[0].fd,VIDIOC_ENUM_FMT,&fmtdesc)!=-1)
    {
    ...
    if(ioctl(buffers[0].fd, VIDIOC_S_FMT, &fmt) == -1)
    {
        printf("Unable to set format\n");
        return FALSE;
    }
    if(ioctl(buffers[0].fd, VIDIOC_G_FMT, &fmt) == -1)
    {
    ...
    if(ioctl(buffers[0].fd, VIDIOC_S_PARM, &setfps) == -1)
    {
        printf("set fps fail !\n");
        return FALSE;
    }
...
/***************************************************/
    ...
    if(ioctl(buffers[0].fd,VIDIOC_REQBUFS,&req)==-1)
    {
    ...
    for (n_buffers = 0; n_buffers < COUNT; n_buffers++)
    {
        ...
        //query buffers
        if (ioctl (buffers[0].fd, VIDIOC_QUERYBUF, &buf) == -1)
        {
        ...
        //map
        buffers[n_buffers].start = mmap(NULL,buf.length,PROT_READ |PROT_WRITE, MAP_SHARED, buffers[0].fd, buf.m.offset);
    ...
    int type = 1;
    ioctl (buffers[0].fd, VIDIOC_STREAMON, &type) ;
...
{
    // loop start
    ioctl(buffers[0].fd, VIDIOC_DQBUF, &dequeue) ; // get frame

    srcBuffer = (unsigned char *)buffers[dequeue.index].start;

    ioctl(buffers[0].fd, VIDIOC_QBUF, &dequeue) ;
    return srcBuffer;
}
...
{
     int type = 0;
     ioctl(buffers[0].fd, VIDIOC_STREAMOFF, &type);

     for(int i=0;i<COUNT;i++)
     {
         munmap(buffers[i].start,BUFFER_SIZE_src);
     }
     if(buffers[0].fd != -1)
     {
         close(buffers[0].fd);
