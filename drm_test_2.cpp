...
int main()
{
    ...
    int num_test_planes = test_crtc->num_planes; // num_test_planes = 4
    for (int i = 0; i < num_test_planes; i++) {
        plane[i] = get_sp_plane(dev, test_crtc); // get planes that isn't used
        if (!plane[i]) {
        ...
        memset(plane[i]->bo->map_addr ,255 ,plane[i]->bo->size) ;  // initialize to 0
    }
