...
static int get_supported_format(struct sp_plane *plane, uint32_t *format, uint32_t _format)
{
    uint32_t i;

    for (i = 0; i < plane->plane->count_formats; i++) {
        if (plane->plane->formats[i] == _format) {
...
struct sp_dev *create_sp_dev(int card, uint32_t _format)
{
    struct sp_dev *dev;
    int ret, fd, i, j;
    drmModeRes *r = NULL;
    ...
    dev = (sp_dev *)calloc(1, sizeof(*dev));
    if (!dev) {
    ...
    dev->connectors = (sp_connector *)calloc(dev->num_connectors,
    ...
    dev->encoders = (_drmModeEncoder **)calloc(dev->num_encoders, sizeof(*dev->encoders));
    if (!dev->encoders) {
    ...
    dev->crtcs = (sp_crtc *)calloc(dev->num_crtcs, sizeof(struct sp_crtc));
    if (!dev->crtcs) {
    ...
    dev->planes = (sp_plane *)calloc(dev->num_planes, sizeof(struct sp_plane));
    for(i = 0; i < dev->num_planes; i++) {
        ...
        ret = get_supported_format(plane, &plane->format, _format);
        if (ret) {
