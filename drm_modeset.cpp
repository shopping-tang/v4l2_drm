            ...
            struct sp_connector *conn, drmModeModeInfoPtr mode)
{
    ...
    pset = drmModeAtomicAlloc(); // instead of drmModeSetCrtc(crtc_id, fb_id, conn_id, &mode)   begin:
    if (!pset) {
...
{
    int ret, i, j;
    unsigned crtc_mask = 0; // unsigned int ?
    ...
    for (i = 0; i < dev->num_connectors; i++) {
        ...
        ret = set_crtc_mode(dev, cr, c, m); // here ok then we can display FB 
        if (ret) {
...
int set_sp_plane(struct sp_dev *dev,struct sp_plane *plane,struct sp_crtc *crtc,uint32_t _x,uint32_t _y,uint32_t _w,uint32_t _h)
{
    int ret;
    uint32_t w, h;
    ...
    ret = drmModeSetPlane(dev->fd, plane->plane->plane_id, // display FrameBuffer
            crtc->crtc->crtc_id, plane->bo->fb_id, 0, _x, _y, _w, _h,
            0, 0, w << 16, h << 16);
    if (ret) {
